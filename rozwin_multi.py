...
class dana_wiersz:
    ...
    def wyswietl(self):
        ...
        lbl_nazwa     = tk.Label (master=self.ramka, text=self.nazwa)
        edt_wartosc   = tk.Entry (master=self.ramka, width=10)
        lbl_jednostka = tk.Label (master=self.ramka, text=self.jednostka)
        
        lbl_nazwa.grid     (row=self.wiersz, column=0, sticky="w", padx=5, pady=5)        
        edt_wartosc.grid   (row=self.wiersz, column=1, sticky="w")
        lbl_jednostka.grid (row=self.wiersz, column=2, sticky="w", padx=5)
        
        edt_wartosc.insert(0, self.wartosc)
        lbl_wybor["text"] = edt_wartosc.get()

        
    def zwroc_dana(self):
        return edt_wartosc.get()
...
lbl_Dd.grid(row=1, column=0, sticky="w", padx=5, pady=5)
